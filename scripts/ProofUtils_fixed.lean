...
def generalize_constants_body {φ : Form N} {x : SVAR} (i : NOM N) (h : x ≥ φ.new_var) (pf : Proof φ) :
    Proof (φ[x // i]) := by
...
def generalize_constants_thm {φ : Form N} {x : SVAR} (i : NOM N) (h : x ≥ φ.new_var)
  ...
  lemma formulasIn_cast {α β : Form N} (h : α = β) (pf : Proof α) :
      (h ▸ pf).formulasIn = pf.formulasIn := by
    subst h; rfl

  private lemma gc_body_general_pos {ψ : Form N} {v : SVAR} {x : SVAR} (old : NOM N)
      (pf : Proof ψ) (h : x ≥ (all v, ψ).new_var) (ih_h : x ≥ ψ.new_var)
      (hc : (v + 1).letter > (Form.new_var ψ).letter) :
      generalize_constants_body old h (general v pf) =
        general v (generalize_constants_body old ih_h pf) := by
    simp only [generalize_constants_body, hc, if_pos hc, ↓reduceIte]
    rfl

  private lemma gc_body_general_neg {ψ : Form N} {v : SVAR} {x : SVAR} (old : NOM N)
      (pf : Proof ψ) (h : x ≥ (all v, ψ).new_var) (hxψ : x ≥ ψ.new_var)
      (hc : ¬ (v + 1).letter > (Form.new_var ψ).letter) :
      generalize_constants_body old h (general v pf) =
        general v (generalize_constants_body old hxψ pf) := by
    simp only [generalize_constants_body, hc, if_neg hc, ↓reduceIte]
    rfl

  private lemma gc_body_necess {ψ : Form N} {x : SVAR} (old : NOM N) (pf : Proof ψ)
      (h : x ≥ (□ψ).new_var) (ih_h : x ≥ ψ.new_var) :
      generalize_constants_body old h (necess pf) =
        necess (generalize_constants_body old ih_h pf) := by
    simp only [generalize_constants_body, ↓reduceIte]
    rfl

  private lemma gc_formulasIn {φ : Form N} {x : SVAR} (old : NOM N) (h : x ≥ φ.new_var) (pf : Proof φ) :
      (generalize_constants old h pf).formulasIn =
        (all x, φ[x // old]) :: (generalize_constants_body old h pf).formulasIn := by
    unfold generalize_constants
    simp only [Proof.formulasIn, Proof.general]

  private lemma mem_formulasIn_gc_body {ψ : Form N} {x : SVAR} (old : NOM N) (pf : Proof ψ)
      (h : x ≥ ψ.new_var) {χ : Form N}
      (hmem : χ ∈ (generalize_constants_body old h pf).formulasIn) :
      χ ∈ (generalize_constants old h pf).formulasIn := by
    unfold generalize_constants
    simp only [Proof.formulasIn, Proof.general]
    exact List.mem_cons.mpr (Or.inr hmem)

  private lemma mem_proof_noms_gc_witness {ψ : Form N} {x : SVAR} (old : NOM N) (pf : Proof ψ)
      (h : x ≥ ψ.new_var) {χ : Form N} {k : NOM N}
      (hmem : χ ∈ (generalize_constants_body old h pf).formulasIn) (hkχ : k ∈ χ.list_noms) :
      k ∈ (generalize_constants old h pf).proof_noms := by
    simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
    exact ⟨χ, mem_formulasIn_gc_body old pf h hmem, hkχ⟩

  private lemma mem_proof_noms_of_mem_proof_noms_general {α : Form N} (v : SVAR) (pf : Proof α) {k : NOM N}
      (hk : k ∈ pf.proof_noms) : k ∈ (general v pf).proof_noms := by
    have hk' := hk
    ...
    exact ⟨χ, List.mem_cons.mpr (Or.inr hχ), hkχ⟩

  private lemma mem_proof_noms_of_mem_proof_noms_necess {α : Form N} (pf : Proof α) {k : NOM N}
      (hk : k ∈ pf.proof_noms) : k ∈ (necess pf).proof_noms := by
    have hk' := hk
    simp only [proof_noms, List.mem_dedup, List.mem_flatMap] at hk'
    obtain ⟨χ, hχ, hkχ⟩ := hk'
    simp only [proof_noms, List.mem_dedup, List.mem_flatMap, Proof.formulasIn]
  ...
  private lemma not_mem_old_head_all {φ : Form N} {x : SVAR} (old : NOM N) (h : x ≥ φ.new_var)
      {χ : Form N} (heq : χ = all x, φ[x // old]) (hkχ : old ∈ χ.list_noms) : False := by
  ...
  private theorem not_mem_proof_noms_generalize_constants {φ : Form N} {x : SVAR} (new old : NOM N)
    ...
    | @general ψ v pf ih =>
        ...
        · simp [hc] at h
          ...
          | inr htail =>
            have hχ' := htail
            simp only [hc, if_pos hc, generalize_constants, generalize_constants_body, Proof.formulasIn, Proof.general, List.mem_cons, id, ↓reduceIte] at hχ'
            rcases List.mem_cons.mp hχ' with heq | hmem''
            · exact (list_noms_nom_subst_svar hφ (heq ▸ hkχ)).2 rfl
            · exact absurd (by
                simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
                exact ⟨χ, mem_formulasIn_gc_body old pf ih_h hmem'', hkχ⟩) (ih ih_h)
        · simp [hc] at h
          ...
          | inr htail =>
            have hχ' := htail
            simp only [hc, if_neg hc, generalize_constants, generalize_constants_body, Proof.formulasIn, Proof.general, List.mem_cons] at hχ'
            rcases List.mem_cons.mp hχ' with heq | hmem''
            · exact (list_noms_nom_subst_svar hφ (heq ▸ hkχ)).2 rfl
            · exact absurd (by
                simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
                exact ⟨χ, mem_formulasIn_gc_body old pf hxψ hmem'', hkχ⟩) (ih hxψ)
    | @necess ψ pf ih =>
        unfold generalize_constants
        have hφ := h
        ...
        | inr htail =>
          have hχ' := htail
          simp only [Proof.formulasIn, Proof.necess, Proof.general, generalize_constants, generalize_constants_body, id, ↓reduceIte] at hχ'
          rcases List.mem_cons.mp hχ' with heq1 | hmem'
          · exact (list_noms_nom_subst_svar hφ (heq1 ▸ hkχ)).2 rfl
          · exact absurd (by
                simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
                exact ⟨χ, mem_formulasIn_gc_body old pf ih_h hmem', hkχ⟩) (ih ih_h)
    | @mp φ₁ ψ₁ pf₁ pf₂ ih1 ih2 =>
        ...
        | inr htail =>
          exact absurd (by
            simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
            exact ⟨χ, mem_formulasIn_gc_body old (mp pf₁ pf₂) h htail, hkχ⟩)
            (not_mem_proof_noms_generalize_constants new old h hne (mp pf₁ pf₂))
    | ax_k =>
        unfold generalize_constants
        intro hk
        dsimp [proof_noms, formulasIn] at hk
    ...
    | ax_q2_svar _ _ _ _ =>
        ...
        | inr h1 =>
          ...
          have heq2 : χ = _ := by simpa [List.mem_singleton, formulasIn_cast] using h1
  ...
  private theorem mem_proof_noms_generalize_constants {φ : Form N} {x : SVAR} (old : NOM N)
    ...
    | @general ψ v pf ih =>
        ...
        · simp [hc] at h
          ...
          | inr htail =>
            have hmem := htail
            simp only [hc, if_pos hc, generalize_constants, generalize_constants_body, Proof.formulasIn,
        ...
        · simp [hc] at h
          ...
          | inr htail =>
            have hmem := htail
            simp only [hc, if_neg hc, generalize_constants, generalize_constants_body, Proof.formulasIn,
              Proof.general, List.mem_cons] at hmem
            rcases List.mem_cons.mp hmem with heq | hmem'
            · rw [heq] at hkχ
              exact Or.inl (mem_proof_noms_of_subst_list_noms old hφ (general v pf) hkχ)
    ...
    | @necess ψ pf ih =>
        ...
        | inr htail =>
          have hmem := htail
          simp only [Proof.formulasIn, Proof.necess, Proof.general, generalize_constants,
            generalize_constants_body, id, ↓reduceIte] at hmem
          rcases List.mem_cons.mp hmem with heq | hmem'
          · rw [heq] at hkχ
            exact Or.inl (mem_proof_noms_of_subst_list_noms old hφ (necess pf) hkχ)
          · rcases ih ih_h (mem_proof_noms_gc_witness old pf ih_h hmem' hkχ) with h | h
    ...
    | @mp φ₁ ψ₁ pf₁ pf₂ ih1 ih2 =>
        ...
        | inl heq =>
          rw [heq] at hkχ
          exact Or.inl (mem_proof_noms_of_subst_list_noms old hφ (mp pf₁ pf₂) hkχ)
        | inr htail =>
          rcases mem_proof_noms_generalize_constants old h (mp pf₁ pf₂)
              (by
                simp only [proof_noms, List.mem_dedup, List.mem_flatMap]
                exact ⟨χ, mem_formulasIn_gc_body old (mp pf₁ pf₂) h htail, hkχ⟩) with hmem | heq
          · exact Or.inl hmem
          · exact Or.inr heq
    | ax_k =>
        unfold generalize_constants at hk
    ...
    | ax_name v =>
        ...
        | inl heq1 =>
          rw [heq1] at hkχ
          exact Or.inl (mem_proof_noms_of_subst_list_noms old h (ax_name v)
            (list_noms_nom_subst_svar h hkχ).left)
        | inr h1 =>
          dsimp [generalize_constants_body] at h1
          have heq2 : χ = _ := by simpa [List.mem_singleton] using h1
          rw [heq2, Form.list_noms] at hkχ
