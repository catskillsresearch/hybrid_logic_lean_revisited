...
section Lemmas
  ...
  theorem generalize_not_free (h1 : is_free v phi = false) :  |=  (phi  <->  (all v, phi)) := by
    intro M s g
    rw [iff_sat]
    apply Iff.intro
    . intro h2
      ...
      | bind u psi ih =>
          ...
          . intro nfree_in_psi
            rw [bind_comm]
            intros g' variant_u
            exact ih nfree_in_psi s g' (h2 g' variant_u)
    . intro h
      exact h g (is_variant_refl v)

  theorem svar_substitution {phi : Form N} {g g' : I M.W} (h_subst : is_substable phi y x) (h_var : is_variant g g' x) (h_which_var : g' x = g y) :
  (((M,s,g)  |=  phi[y // x])  <->  (M,s,g')  |=  phi) := by
    induction phi generalizing s g g' with
    | svar z   =>
        by_cases z_x : z = x
    ...
    | impl psi chi ind_hyp_1 ind_hyp_2 =>
    ...
    | bind v psi ind_hyp =>
        cases x_free : is_free x psi with
        | true =>
            by_cases x_v : x = v
            . -- all x, psi             and       x is free in psi
              simp only [subst_svar, x_v, ite_true]
              rw [ <- x_v]
              have := @generalize_not_free x N (all x, psi) (by simp [is_free])
              apply Iff.intro
              . intro h
                have := this M s g
                rw [iff_sat] at this
            ...
            . by_cases y_v : y = v
              . -- all y, psi          and       x is free in psi
                -- contradiction with h_subst:
                simp [is_substable, x_free, y_v] at h_subst   
              . --  all v, psi  (v  !=  x and v  !=  y) and x is free in psi
                ...
                . intro h1
                  ...
                  -- Since f is a v-variant of g and we have (M,s,g) |= all v, psi[y//x],
                  --  we'll obtain (M,s,f) |= psi[y//x].
                  ...
                  | <f, f_var_g_v, f_var_f'_x> =>
                      have t1 : f' x = f y := by
                        ...
                        assumption
                      have t2 : (M,s,f)  |=  psi[y//x] := h1 f (is_variant_symm.mpr f_var_g_v)
                ...
                . intro h1
                  ...
                  | <f', f_var_f'_x, f'_var_g'_v> =>
                    have t1 : f' x = f y := by
                        ...
                        rw [show f' x = g' x from f'_var_g'_v x (Ne.symm (Ne.intro x_v))]
                        assumption
                    have t2 : (M,s,f')  |=  psi := h1 f' f'_var_g'_v
                    exact (@ind_hyp s f f' h_subst f_var_f'_x t1).mpr t2
        | false =>
            have x_nfree : is_free x (all v, psi) = false := preserve_notfree x v x_free
    ...
    theorem nom_substitution {phi : Form N} {x : SVAR} {i : NOM N} {g g' : I M.W}
    (h_var : is_variant g g' x) (h_which_var : g' x = M.V_n i) :
    (((M,s,g)  |=  phi[i // x])  <->  ((M,s,g')  |=  phi)) := by
      induction phi generalizing s g g' with
      | svar y =>
          by_cases x_y : x = y
          . apply Iff.intro
      ...
      | bind y psi ih =>
          ...
          . rw [if_neg x_y]
            ...
            . intro h1
              intro f' f'_var_g'_y
              have t1 : f' x = Model.V_n M i := Eq.trans (f'_var_g'_y x (Ne.symm x_y)) h_which_var
              have exists_mirror := variant_mirror_property g g' f' h_var (is_variant_symm.mp f'_var_g'_y)
              match exists_mirror with
              | <f, g_var_f_y, f_var_f'_x> =>
                  have t2 : (M,s,f)  |=  psi[i//x] := h1 f (is_variant_symm.mp g_var_f_y)
            ...
            . intro h2
              ...
              | <f', g'_var_f'_y, f'_var_f_x> =>
                  have t1 : f' x = Model.V_n M i := by
                    rw [ <-  g'_var_f'_y x (Ne.symm x_y), h_which_var]
                  have t2 : (M,s,f')  |=  psi := h2 f' (is_variant_symm.mp g'_var_f'_y)
                  exact (@ih s f f' (is_variant_symm.mp f'_var_f_x) t1).mpr t2
      | _ => simp [subst_nom, Sat]

  theorem sat_iterated_nec : ((M,s,g)  |=  iterate_nec n phi)  <->  (forall  s' : M.W, (path M.R s s' n)  ->  (M,s',g)  |=  phi) := by
  ...
  theorem sat_iterated_pos : ((M,s,g)  |=  iterate_pos n phi)  <->  (exists  s' : M.W, (path M.R s s' n)  /\  (M,s',g)  |=  phi) := by
    induction n generalizing phi with
    | zero   =>
        ...
        . intro h
          match h with
          | <s', s_s', s'_sat_phi> => rw [s_s'] ; exact s'_sat_phi
    | succ m ih =>
        apply Iff.intro
        . intro h1
          ...
          have by_ih := ih.mp h1
          match by_ih with
          | <s', ex_path1, s'_pos_phi> => 
            rw [pos_sat] at s'_pos_phi
            match s'_pos_phi with
              | <s'', s'_R_s'', s''_phi> => 
                exists s''
                exact <<s', s'_R_s'', ex_path1>, s''_phi>
        . intro h2
          rw [iter_pos_succ]
          unfold path at h2
          match h2 with
          | <s', exist, s'_phi> =>
            match exist with
            | <s'', s''_R_s', ex_path2> =>
              have s''_pos_phi : (M,s'',g)  |=  <>  phi := by rw [pos_sat] ; exists s'
              have premise : exists  s'', path M.R s s'' m  /\  (M,s'',g) |= <>  phi := <s'', <ex_path2, s''_pos_phi>>
              exact ih.mpr premise

  theorem svar_unique_state {v : SVAR} :
  (((M,s,g)  |=  Form.svar v)  ->  (forall  r : M.W, ((M,r,g)  |=  Form.svar v)  ->  r = s)) := by
    intro h1 r h2
    rw [h2, h1]
end Lemmas

section Tautologies
  noncomputable def model_val_func (M : Model N) (s : M.W) (g : I M.W) : Form N  ->  Bool := fun  phi => ite ((M,s,g)  |=  phi) true false

  noncomputable def model_eval (M : Model N) (s : M.W) (g : I M.W) : Eval N :=
      let f := model_val_func M s g
      have p1 : f False = false := by simp [f, model_val_func]
      have p2 : forall  phi psi : Form N, (f (phi  -->  psi) = true)  <->  (not (f phi) = true  \/  (f psi) = true) := fun  phi psi : Form N => by simp [f, model_val_func] 
      <f, p1, p2>

  theorem taut_sound : Tautology phi  ->   |=  phi := by
    intro h M s g
...
theorem WeakSoundness : ( |-  phi)  ->  ( |=  phi) := by
  intro pf
  induction pf with
  ...
  | ax_q2_svar _ x y h_subst =>
      ...
      let g' : I M.W := fun  v => ite (v  !=  x) (g v) (g y)
  ...
  | ax_q2_nom _ x i =>
      intro M s g
      intro h
      let g' : I M.W := fun  v => ite (v  !=  x) (g v) (M.V_n i)
      have h_var : is_variant g g' x := by
        intro v x_not_v
        simp [g', Ne.symm x_not_v]
      have h_which_var : g' x = M.V_n i := by simp [g']
  ...
  | ax_name v =>
      intro M s g
      rw [ex_sat]
      let g' : I M.W := fun  x => ite (v = x) s (g x)
      apply Exists.intro
      . apply And.intro
  ...
  | ax_nom n m =>
      ...
      | <s', _, s'_sat> =>
          ...
          have s'_sat_phi := s'_sat.right
          have s''_is_s' := svar_unique_state s'_sat_v s'' s''_sat_v
          rw [s''_is_s']
          exact s'_sat_phi
  
  | @ax_brcn phi v =>
      intro M s g (h : (M,s,g)  |=  all v, [] phi) s' sRs' g' g_var_g'_v
...
theorem Soundness : (Gamma  |-  phi)  ->  (Gamma  |=  phi) := by
...
theorem Consistency : forall  {N : Set Nat},  |/-  (@Form.bttm N) := by
  intro N habs
  have bot_valid := WeakSoundness habs
  let M : Model N := <Nat, fun  _ => fun  _ => True, fun  _ => {},  fun  _ => 0>
  have g : I (M.W) := fun  _ => 0
  have := bot_valid M 0 g
  exact this

theorem npf_negpf :  |-  (~phi)  ->   |/-  phi := by
  intro h habs
  have := Proof.mp h habs
  exact Consistency this

theorem pos_npf_not :  |- (<> phi)  ->   |/- (~phi) := by
  rw [Form.diamond]
  intro h habs
